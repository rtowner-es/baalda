...
/// Raw contents of the open vault's `.opencontext/config.json`, or None if absent.
...
/// app profile (spec 03 §5 "store server vault id in .opencontext/config.json").
#[tauri::command]
pub async fn get_vault_config(state: State<'_, AppState>) -> AppResult<Option<String>> {
    let (vault, _) = require_vault(&state)?;
    let p = vault.join(".opencontext").join("config.json");
...
/// Overwrite the open vault's `.opencontext/config.json` with `content`.
#[tauri::command]
pub async fn set_vault_config(state: State<'_, AppState>, content: String) -> AppResult<()> {
    let (vault, _) = require_vault(&state)?;
    let dir = vault.join(".opencontext");
    std::fs::create_dir_all(&dir)?;
